  ...
  const renderContent = () => {
    ...
    const imageDisplay = (
      <div className="relative">
        ...
        <img
            ...
            className={`absolute top-0 left-0 w-full h-auto object-contain max-h-[60vh] rounded-xl transition-opacity duration-200 ease-in-out ${isComparing ? 'opacity-0' : 'opacity-100'} ${activeTab === 'retouch' && selectedModel === 'gemini' ? 'cursor-crosshair' : ''}`}
        />
      </div>
    );
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        <div className="w-full max-w-sm mx-auto bg-gray-800/80 border border-gray-700/80 rounded-lg p-1 flex items-center justify-center gap-1 backdrop-blur-sm">
            {(['gemini', 'fal'] as Model[]).map(model => (
              <button
                key={model}
                onClick={() => setSelectedModel(model)}
                className={`w-full font-semibold py-2 px-4 rounded-md transition-all duration-200 text-sm ${
                  selectedModel === model 
                  ? 'bg-gradient-to-br from-blue-500 to-cyan-400 text-white shadow-md shadow-cyan-500/20' 
                  : 'text-gray-300 hover:text-white hover:bg-white/10'
                }`}
              >
                {model === 'gemini' ? 'Gemini' : 'Fal Qwen Edit'}
              </button>
            ))}
        </div>
        ...
        <div className="relative w-full shadow-2xl rounded-xl overflow-hidden bg-black/20">
            ...
            {displayHotspot && !isLoading && activeTab === 'retouch' && selectedModel === 'gemini' && (
                <div 
